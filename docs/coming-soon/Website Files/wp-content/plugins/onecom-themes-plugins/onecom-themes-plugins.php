...
if ( ! defined( 'ONECOM_WP_VERSION' ) ) {
	define( 'ONECOM_WP_VERSION', '4.4.7' );
}
...
if ( ! function_exists( 'one_core_admin' ) ) {
	function one_core_admin() {
		...
		add_submenu_page(
			'', // Adding empty to hide from submenu.
			__( 'Plugins', 'onecom-wp' ),
			__( 'Plugins', 'onecom-wp' ),
			'manage_options',
			'onecom-wp-recommended-plugins',
			'one_core_recommended_plugin_listing_callback'
		);
		add_submenu_page(
			'', // Adding empty to hide from submenu.
			__( 'Plugins', 'onecom-wp' ),
			...
			'one_core_discouraged_plugin_listing_callback'
		);
		add_submenu_page(
			OC_PLUGIN_DOMAIN,
			__( 'Themes', 'onecom-wp' ),
