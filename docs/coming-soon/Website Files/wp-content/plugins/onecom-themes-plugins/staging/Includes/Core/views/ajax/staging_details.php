				...
				<div class="one-card-action-old staging-details-created">
					...
					if ( ! empty( $clones ) ) :
						foreach ( $clones as $key => $clone ) :
							...
							<div class="wrap-rgh-btn"><a href="javascript:void(0);" data-loginUrl="<?php echo trailingslashit( $clone['url'] ); ?>wp-login.php" data-stgUrl="<?php echo trailingslashit( $clone['url'] ); ?>" class="one-button btn button_1 loginStaging" style="min-width: 62px;text-align: center;"><?php _e( 'Log in to staging', 'onecom-wp' ); ?></a></div>
							<div class="wrap-rgh-btn"><a href="<?php echo $clone['url']; ?>" target="_blank" class="viewStaging"><img src="<?php echo ONECOM_WP_URL . 'assets/images/view-site.svg'; ?>" alt="View your site" class="action-rht-img" /><span><?php _e( 'View your site', 'onecom-wp' ); ?></span></a></div>
							<?php } ?>
							<?php
						endforeach;
					endif;
					if ( empty( $clones ) || $cloneExists ) {
						echo $rebuild_btn = '<div class="wrap-rgh-btn"><a href="javascript:void(0);" class="one-button btn one-button-update-staging" data-staging-id="" data-dialog-id="staging-update-confirmation" data-title="' . __( 'Are you sure?', 'onecom-wp' ) . '" data-width="500" data-height="300"><img src="' . ONECOM_WP_URL . 'assets/images/rebuild-staging.svg" alt="Rebuild staging" class="action-rht-img" /><span>' . __( 'Rebuild staging', 'onecom-wp' ) . '</span></a></div>';
					} else {
						echo $rebuild_btn = '<div class="wrap-rgh-btn"><a href="javascript:void(0);" class="one-button btn one-button-update-staging rebuild-btn" data-staging-id="" data-dialog-id="staging-update-confirmation" data-title="' . __( 'Are you sure?', 'onecom-wp' ) . '" data-width="500" data-height="300"><span>' . __( 'Rebuild staging', 'onecom-wp' ) . '</span></a></div>';
					}
					$delete_btn = '<div class="wrap-rgh-btn"><a href="javascript:;" class="staging-trash one-button-delete-staging"  title="' . __( 'Delete Staging', 'onecom-wp' ) . '" data-title="' . __( 'Are you sure?', 'onecom-wp' ) . '" data-dialog-id="staging-delete" data-width="500" data-height="275"><img src="' . ONECOM_WP_URL . 'assets/images/delete-staging.svg" alt="Delete staging" class="action-rht-img" /><span>' . __( 'Delete staging', 'onecom-wp' ) . '</span></a></div>';
					echo $delete_btn;
					?>
					<div class="wrap-rgh-btn"><a href="<?php echo onecom_generic_locale_link( $request = 'staging_guide', get_locale() ); ?>" target="_blank" class="help_link2"><img src="<?php echo ONECOM_WP_URL . 'assets/images/need-help.svg'; ?>" alt="Need help" class="action-rht-img" /><span><?php _e( 'Need help?', 'onecom-wp' ); ?></span></a></div>
				</div>
			</div>
		</div>
	</div>
