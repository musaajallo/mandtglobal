...
$GLOBALS['WpvPushMenuVersion'] = '2.0.19';
...
class WpvPushMenu {
	...
	private function load_menu() {
		...
		if ( has_nav_menu( $this->menu_name ) && $locations = get_nav_menu_locations() ) {
			...
			foreach ( $sorted_menu_items as $key => $menu_item ) {
				...
				if ( isset( $menu_items_by_id[(int)$menu_item->menu_item_parent] ) ) {
			...
			wp_localize_script( 'vamtam-push-menu', 'WpvPushMenu', array(
				...
				'limit'  => function_exists( 'wpv_get_option' ) ? wpv_get_option( 'mobile-top-bar-resolution' ) : 768,
			) );
	...
	private function get_wpml_translations() {
		if (
			...
			apply_filters( 'vamtam_push_menu_disable_wpml', false )
		) {
			return false;
		}
