	...
	class Onecom_Usage_Stats {
		...
		function check_status() {
			if ( get_site_option(self::OC_CU_CONSENT) &&
				...
				// update timestamp
				update_site_option(self::OC_CU_CONSENT, current_time('timestamp'), 'no');
				// push stats
		...
		function notice_submission() {
			if ( isset($_POST['oc-save-consent']) ) {
				if ( isset($_POST[ self::OC_CU_CONSENT ]) && $_POST[ self::OC_CU_CONSENT ] == 'on' ) {
					if ( update_site_option(self::OC_CU_CONSENT, current_time('timestamp'), 'no') ) {
						// push stats
						$this->prepare_payload();
					}
				} else {
					update_site_option(self::OC_CU_CONSENT, 'false', 'no');
